(function ($) {
	$.fn.responsiveCollapse = function(options) {
		...
		return this.filter('ul').each(function() {
			...
			var overflowItem = $('<li class="dropdown dots-wrap hide"><a href="#" class="dropdown-toggle menu-link" data-toggle="dropdown">' + settings.overflowItemText + '</a><ul class="dropdown-menu dropdown-menu-right list-unstyled"></ul></li>').appendTo($list);
			...
			function refresh() {
				...
				if( $(".dots-wrap").length ){
					$("#navbar_greedy li").removeClass('last-menu-li');
					$(".dots-wrap").prev().addClass('last-menu-li');
				}
			}
